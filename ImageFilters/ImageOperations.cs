...
namespace ImageFilters
{
    ...
    public static class ImageOperations
    {
        ...
        public static byte Alphatrimfilter(byte[,] ImageMatrix, int x, int y, int Wmax, int Sort)
        {
            ...
            for (int i = 0; i < Wmax * Wmax; i++)
            {
             ...
             else if (Sort == 3) Array = SELECTION_SORT(Array, ArrayLength);
        ...
        public static byte[,] ImageFilter(byte[,] ImageMatrix, int Max_Size, int Sort, int filter)
        {
            ...
            for (int y = 0; y < GetHeight(ImageMatrix); y++)
            {
                ...
                {
                    if (filter == 1)
                        ImageMatrix2[y, x] = Alphatrimfilter(ImageMatrix, x, y,Max_Size, Sort);
                   else if (filter==2)
